(function () {
    ...
    this.ShotWheel = function (drawingCanvas, statusLabel, users, callback) {
        ...
        function initDrawingCanvas() {
            ...
            drawingCanvas.addEventListener('touchcancel', checkEndDrag);
        }
        ...
        function update() {
            ...
            if (wheelSpinning === true && wheelStopped === false &&
                ...
                var currentSegment = wheel.getSegmentPosition();
...
/////////////////////////////
        ...
        Wheel.prototype = {
            ...
            //     for (var i = 0; i < l; i++) {
            //         var xCord = ((i+0.5) / l * TWO_PI);
            //         var yCord = ((i+0.5) / l * TWO_PI);
            //         console.log(xCord, yCord);
            //         var x = Math.cos(xCord) * 5,
            //             y = Math.sin(yCord) * 5;

            //         this.body.addShape(pin, [x, y]);
            //         this.pNamePositions[i] = [x * ppm, -y * ppm ];
            //     }
            // },
            getSegmentPosition: function () {
                var currentRotation = wheel.body.angle % TWO_PI,
                    currentSegment = Math.floor(currentRotation / this.deltaPI);

                // console.log(currentRotation);

                return Math.abs(currentSegment);
            },
            draw: function () {
                ...
                for (var i = 0; i < this.segments; i++) {
                    ...
                    ctx.beginPath();
                    ctx.arc(0, 0, this.pRadius, i * this.deltaPI, (i + 1) * this.deltaPI, false);

                    // ctx.arc(0, 0, this.pRadius, i * this.deltaPI, (i + 1) * this.deltaPI);
                    // ctx.arc(0, 0, this.pRadius, i * this.deltaPI + calculation, (i + 1) * this.deltaPI + calculation);
                    //ctx.arc(0, 0, this.pRadius, i * (TWO_PI / this.segments) * ((10 / this.segments) + 1), (i + 1) * (TWO_PI / this.segments) * ((10 / this.segments) + 1));
                    ctx.lineTo(0, 0);
                ...
                // this.pNamePositions.forEach(function(p) {
                //     ctx.beginPath();
                //     // ctx.rotate(TWO_PI / (1*6));
                //     ctx.fillText("maiko", p[0], p[1]);
                //     ctx.fill();
                // }, this);
